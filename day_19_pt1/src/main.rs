...
fn main() {
    ...
    while !finished {
        let simple_step = take_step(&entry_hash, &mut travel_state, &mut seen);

        if !simple_step {
            let turn_step = turn_step(&entry_hash, &mut travel_state);

            if turn_step {
                let turned_step = take_step(&entry_hash, &mut travel_state, &mut seen);
                if !turned_step {
                    finished = true;
                }
            } else {
    ...
    let result: String = seen.into_iter().collect();
    println!("{}", result);
}
...
fn take_step(entries: &HashMap<usize,char>, travel: &mut TravelState, seen: &mut Vec<char>) -> bool {
